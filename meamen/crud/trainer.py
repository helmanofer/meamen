...
async def get_trainers(session, skip: int = 0, limit: int = 100) -> List[Trainer]:
    statement = select(Trainer).offset(skip).limit(limit)
    result = await session.execute(statement)
    return result.scalars().all()


async def create_trainer(session, trainer: Trainer) -> Trainer:
    session.add(trainer)
...
async def get_trainer_by_id(session, trainer_id: int) -> Optional[Trainer]:
...
async def update_trainer(
    session, trainer_id: int, trainer_data: dict
) -> Optional[Trainer]:
...
async def delete_trainer(session, trainer_id: int) -> bool:
    trainer = await get_trainer_by_id(session, trainer_id)
    if not trainer:
        return False
